...
const JWT_SECRET = process.env.JWT_SECRET || 'campus-wechat-2026';
...
async function initDatabase() {
  ...
  db.run(`CREATE TABLE IF NOT EXISTS admin_tenants (id TEXT PRIMARY KEY, name TEXT, status TEXT, description TEXT DEFAULT '', create_time DATETIME DEFAULT CURRENT_TIMESTAMP)`);
  try {
    ...
    console.error('Seed tenants error:', e);
  }
...
async function ensureChatTables() {
  // 兼容sql.js的CREATE TABLE语法（移除AUTO_INCREMENT/BOOLEAN，改用sql.js支持的语法）
...
function run(sql, params = []) { 
  try {
    ...
    console.error('执行SQL失败:', sql, params, e);
    return { lastID: 0 };
  }
}
...
const adminAuth = (req, res, next) => {
  try {
    ...
    const admins = query('SELECT * FROM admins WHERE id=?', [payload.uid]);
...
// ====================== 新增：微信分享邀请接口 ======================
app.post('/api/friend/invite', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => {
  const { userId, inviteCode } = req.body;
  const inviteUrl = `http://localhost:3000/invite?code=${inviteCode || Math.random().toString(36).slice(2, 10)}`;
  res.json({ 
    code: 200, 
    msg: '生成邀请链接成功', 
    data: { 
      inviteUrl,
      qrCode: `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(inviteUrl)}` 
    } 
  });
});
...
io.on('connection', (socket) => {
  socket.on('login', (uid) => { 
    ...
    socket.uid = uid; 
    run('UPDATE users SET online=1 WHERE id=?', [uid]); 
    io.emit('user_online', { uid, online: true }); 
  });
  ...
  socket.on('sync_offline', () => {
    try {
      const stmt = db.prepare('SELECT id, from_uid, to_uid, content, type, file_url, create_time FROM messages WHERE to_uid=? AND is_read=0');
      stmt.bind([socket.uid]);
      const offline = [];
  ...
  socket.on('send_group_msg', async (data) => {
    ...
    query('SELECT uid FROM group_members WHERE group_id=?', [groupId]).forEach(m => { 
      if (onlineUsers.get(m.uid)) io.to(onlineUsers.get(m.uid)).emit('recv_group_msg', { id: msgId, groupId, fromUid: socket.uid, content, type, fileUrl }); 
    });
  });
  ...
  socket.on('call', (data) => { 
    const toSocketId = onlineUsers.get(data.toUid); 
    if (toSocketId) io.to(toSocketId).emit('incoming_call', { fromUid: socket.uid, callType: data.callType, roomId: data.roomId }); 
  });
  
  socket.on('call_accept', (data) => { 
    const toSocketId = onlineUsers.get(data.toUid); 
    if (toSocketId) io.to(toSocketId).emit('call_accepted', { roomId: data.roomId }); 
  });
  
  socket.on('call_reject', (data) => { 
    const toSocketId = onlineUsers.get(data.toUid); 
    if (toSocketId) io.to(toSocketId).emit('call_rejected'); 
  });
  
  socket.on('disconnect', () => { 
    if (socket.uid) { 
      onlineUsers.delete(socket.uid); 
      run('UPDATE users SET online=0 WHERE id=?', [socket.uid]); 
      io.emit('user_offline', { uid: socket.uid }); 
    } 
  });
});
...
app.get('/api/user/info', auth, (req, res) => { 
  const users = query('SELECT * FROM users WHERE id=?', [req.uid]); 
  res.json({ code: 200, data: users[0] }); 
});

app.post('/api/user/update', auth, (req, res) => { 
  const { nickname, avatar, phone, settings } = req.body; 
  run('UPDATE users SET nickname=?, avatar=?, phone=?, settings=? WHERE id=?', [nickname || '', avatar || '', phone || '', JSON.stringify(settings || {}), req.uid]); 
  res.json({ code: 200, msg: '更新成功' }); 
});

app.post('/api/user/set_status', auth, (req, res) => { 
  const { online } = req.body; 
  run('UPDATE users SET online=? WHERE id=?', [online, req.uid]); 
  io.emit('status_changed', { uid: req.uid, online }); 
  res.json({ code: 200, msg: '状态已更新' }); 
});

app.get('/api/user/search', auth, (req, res) => { 
  const { keyword } = req.query; 
  res.json({ code: 200, data: query('SELECT id, nickname, avatar, online FROM users WHERE nickname LIKE ? AND id!=? LIMIT 20', [`%${keyword || ''}%`, req.uid]) }); 
});

// ====================== 好友 ======================
app.get('/api/friends', auth, (req, res) => { 
  res.json({ code: 200, data: query('SELECT u.id, u.nickname, u.avatar, u.online, u.last_online FROM friends f LEFT JOIN users u ON f.friend_uid=u.id WHERE f.uid=? AND f.status=1', [req.uid]) }); 
});

app.get('/api/friend_requests', auth, (req, res) => { 
  res.json({ code: 200, data: query('SELECT r.*, u.nickname, u.avatar FROM friend_requests r LEFT JOIN users u ON r.from_uid=u.id WHERE r.to_uid=? AND r.status=0', [req.uid]) }); 
});

app.post('/api/friend/add', auth, (req, res) => { 
  const { toUid, message = '' } = req.body; 
  if (toUid == req.uid) return res.json({ code: 400, msg: '不能添加自己' }); 
  run('INSERT INTO friend_requests (from_uid, to_uid, message) VALUES (?,?,?)', [req.uid, toUid, message]); 
  const toSocketId = onlineUsers.get(toUid); 
  if (toSocketId) io.to(toSocketId).emit('new_friend_request', { fromUid: req.uid }); 
  res.json({ code: 200, msg: '申请已发送' }); 
});

app.post('/api/friend/accept', auth, (req, res) => { 
  const { requestId } = req.body; 
  const requests = query('SELECT * FROM friend_requests WHERE id=? AND to_uid=?', [requestId, req.uid]); 
  if (requests.length === 0) return res.json({ code: 400, msg: '请求不存在' }); 
  run('UPDATE friend_requests SET status=1 WHERE id=?', [requestId]); 
  run('INSERT OR IGNORE INTO friends (uid, friend_uid, status) VALUES (?,?,1)', [req.uid, requests[0].from_uid]); 
  run('INSERT OR IGNORE INTO friends (uid, friend_uid, status) VALUES (?,?,1)', [requests[0].from_uid, req.uid]); 
  res.json({ code: 200, msg: '已同意' }); 
});

app.post('/api/friend/reject', auth, (req, res) => { 
  run('UPDATE friend_requests SET status=2 WHERE id=?', [req.body.requestId]); 
  res.json({ code: 200, msg: '已拒绝' }); 
});

app.post('/api/friend/delete', auth, (req, res) => { 
  run('DELETE FROM friends WHERE uid=? AND friend_uid=?', [req.uid, req.body.friendUid]); 
  run('DELETE FROM friends WHERE uid=? AND friend_uid=?', [req.body.friendUid, req.uid]); 
  res.json({ code: 200, msg: '已删除' }); 
});

app.post('/api/friend/block', auth, (req, res) => { 
  run('INSERT OR IGNORE INTO blocked_users (uid, block_uid) VALUES (?,?)', [req.uid, req.body.friendUid]); 
  run('DELETE FROM friends WHERE uid=? AND friend_uid=?', [req.uid, req.body.friendUid]); 
  res.json({ code: 200, msg: '已拉黑' }); 
});

app.get('/api/blocked', auth, (req, res) => { 
  res.json({ code: 200, data: query('SELECT u.id, u.nickname, u.avatar FROM blocked_users b LEFT JOIN users u ON b.block_uid=u.id WHERE b.uid=?', [req.uid]) }); 
});
...
app.get('/api/chat/history', auth, (req, res) => { 
  const { targetId, chatType = 'p2p' } = req.query; 
  const messages = chatType === 'p2p' ? query('SELECT * FROM messages WHERE ((from_uid=? AND to_uid=?) OR (from_uid=? AND to_uid=?)) AND is_del=0 ORDER BY create_time ASC LIMIT 100', [req.uid, targetId, targetId, req.uid]) : query('SELECT * FROM messages WHERE group_id=? AND is_del=0 ORDER BY create_time ASC LIMIT 100', [targetId]); 
...
app.get('/api/chat/unread', auth, (req, res) => { 
  res.json({ code: 200, data: { count: query('SELECT COUNT(*) as cnt FROM messages WHERE to_uid=? AND is_read=0', [req.uid])[0]?.cnt || 0 } }); 
});

// ====================== 群聊 ======================
app.get('/api/groups', auth, (req, res) => { 
  res.json({ code: 200, data: query('SELECT g.* FROM groups g LEFT JOIN group_members gm ON g.id=gm.group_id WHERE gm.uid=? AND g.is_dissolved=0', [req.uid]) }); 
});

app.get('/api/group/members', auth, (req, res) => { 
  res.json({ code: 200, data: query('SELECT u.id, u.nickname, u.avatar, gm.role FROM group_members gm LEFT JOIN users u ON gm.uid=u.id WHERE gm.group_id=?', [req.query.groupId]) }); 
});

app.post('/api/group/dissolve', auth, (req, res) => { 
  const { groupId } = req.body; 
  const groups = query('SELECT * FROM groups WHERE id=? AND owner_uid=?', [groupId, req.uid]); 
  if (groups.length === 0) return res.json({ code: 400, msg: '无权限' }); 
  run('UPDATE groups SET is_dissolved=1 WHERE id=?', [groupId]); 
  run('DELETE FROM group_members WHERE group_id=?', [groupId]); 
  res.json({ code: 200, msg: '群已解散' }); 
});

app.post('/api/group/remove_member', auth, (req, res) => { 
  const { groupId, memberUid } = req.body; 
  const groups = query('SELECT * FROM groups WHERE id=? AND owner_uid=?', [groupId, req.uid]); 
  if (groups.length === 0) return res.json({ code: 400, msg: '无权限' }); 
  run('DELETE FROM group_members WHERE group_id=? AND uid=?', [groupId, memberUid]); 
...
app.get('/api/moments', auth, (req, res) => {
  ...
  res.json({ code: 200, data: moments.map(m => ({ 
    ...m, 
    images: JSON.parse(m.images || '[]'), 
    likes: query('SELECT u.id, u.nickname FROM moment_likes ml LEFT JOIN users u ON ml.uid=u.id WHERE moment_id=?', [m.id]), 
    comments: query('SELECT c.*, u.nickname FROM moment_comments c LEFT JOIN users u ON c.uid=u.id WHERE moment_id=?', [m.id]) 
  })) });
});

app.post('/api/moments/publish', auth, (req, res) => { 
  const { content, images = [] } = req.body; 
  res.json({ code: 200, msg: '发布成功', data: { id: run('INSERT INTO moments (uid, content, images) VALUES (?,?,?)', [req.uid, content, JSON.stringify(images)]).lastID } }); 
});

app.post('/api/moments/like', auth, (req, res) => { 
  const { momentId } = req.body; 
  const exist = query('SELECT id FROM moment_likes WHERE uid=? AND moment_id=?', [req.uid, momentId]); 
  if (exist.length > 0) { 
    run('DELETE FROM moment_likes WHERE uid=? AND moment_id=?', [req.uid, momentId]); 
    run('UPDATE moments SET like_count=like_count-1 WHERE id=?', [momentId]); 
  } else { 
    run('INSERT INTO moment_likes (uid, moment_id) VALUES (?,?)', [req.uid, momentId]); 
    run('UPDATE moments SET like_count=like_count+1 WHERE id=?', [momentId]); 
  } 
  res.json({ code: 200, msg: '操作成功' }); 
});

app.post('/api/moments/comment', auth, (req, res) => { 
  const { momentId, content } = req.body; 
  run('INSERT INTO moment_comments (uid, moment_id, content) VALUES (?,?,?)', [req.uid, momentId, content]); 
  run('UPDATE moments SET comment_count=comment_count+1 WHERE id=?', [momentId]); 
  res.json({ code: 200, msg: '评论成功' }); 
});

// ====================== 跑腿 ======================
app.get('/api/run/orders', auth, (req, res) => { 
  const { status, my, type } = req.query; 
  let sql = 'SELECT r.*, u.nickname as publisher_name, u.avatar as publisher_avatar FROM run_orders r LEFT JOIN users u ON r.uid=u.id WHERE 1=1'; 
  const params = []; 
  if (my === 'true') { 
    sql += ' AND (r.uid=? OR r.runner_uid=?)'; 
    params.push(req.uid, req.uid); 
...
app.post('/api/run/publish', auth, (req, res) => { 
  const { title, detail, fee, fromAddr, toAddr, type = 'run' } = req.body; 
  run('INSERT INTO run_orders (uid, title, detail, fee, from_addr, to_addr, type) VALUES (?,?,?,?,?,?,?)', [req.uid, title, detail, fee, fromAddr, toAddr, type]); 
  res.json({ code: 200, msg: '发布成功' }); 
});

app.post('/api/run/take', auth, (req, res) => { 
  run('UPDATE run_orders SET status=1, runner_uid=? WHERE id=? AND status=0', [req.uid, req.body.orderId]); 
  res.json({ code: 200, msg: '接单成功' }); 
});

app.post('/api/run/finish', auth, (req, res) => { 
  const { orderId } = req.body; 
  const orders = query('SELECT uid, fee FROM run_orders WHERE id=?', [orderId]); 
  if (orders.length === 0) return res.json({ code: 400, msg: '订单不存在' }); 
  run('UPDATE users SET money=money-? WHERE id=?', [orders[0].fee, orders[0].uid]); 
  run('UPDATE users SET money=money+? WHERE id=?', [orders[0].fee, req.uid]); 
  run('UPDATE run_orders SET status=2 WHERE id=?', [orderId]); 
  res.json({ code: 200, msg: '订单完成，费用已结算' }); 
});

// ====================== 骑手 ======================
app.post('/api/rider/auth', auth, (req, res) => { 
  const { name, idCard, phone } = req.body; 
  if (query('SELECT id FROM rider_auth WHERE uid=?', [req.uid]).length > 0) return res.json({ code: 400, msg: '已提交申请' }); 
  run('INSERT INTO rider_auth (uid, name, id_card, phone) VALUES (?,?,?,?)', [req.uid, name, idCard, phone]); 
  res.json({ code: 200, msg: '申请已提交' }); 
});

app.get('/api/rider/rank', auth, (req, res) => { 
  res.json({ code: 200, data: query('SELECT u.id, u.nickname, u.avatar, COUNT(r.id) as order_count FROM users u LEFT JOIN run_orders r ON r.runner_uid=u.id AND r.status=2 WHERE r.id IS NOT NULL GROUP BY u.id ORDER BY order_count DESC LIMIT 10') }); 
});

// ====================== 商家 ======================
app.post('/api/shop/apply', auth, (req, res) => { 
  const { name, phone, address } = req.body; 
  if (query('SELECT id FROM shops WHERE uid=?', [req.uid]).length > 0) return res.json({ code: 400, msg: '已有店铺' }); 
  run('INSERT INTO shops (uid, name, phone, address) VALUES (?,?,?,?)', [req.uid, name, phone, address]); 
  res.json({ code: 200, msg: '申请已提交' }); 
});

app.get('/api/shops', auth, (req, res) => { 
  res.json({ code: 200, data: query('SELECT * FROM shops WHERE status=1') }); 
});

// ====================== 地址 ======================
app.get('/api/addresses', auth, (req, res) => { 
  res.json({ code: 200, data: query('SELECT * FROM addresses WHERE uid=? ORDER BY is_default DESC', [req.uid]) }); 
});

app.post('/api/addresses/add', auth, (req, res) => { 
  const { name, phone, address, isDefault } = req.body; 
  if (isDefault) run('UPDATE addresses SET is_default=0 WHERE uid=?', [req.uid]); 
  run('INSERT INTO addresses (uid, name, phone, address, is_default) VALUES (?,?,?,?,?)', [req.uid, name, phone, address, isDefault ? 1 : 0]); 
  res.json({ code: 200, msg: '添加成功' }); 
});

// ====================== 支付 ======================
app.get('/api/pay/wallet', auth, (req, res) => { 
  res.json({ code: 200, data: { money: query('SELECT money FROM users WHERE id=?', [req.uid])[0]?.money || 0 } }); 
});

app.post('/api/pay/recharge', auth, (req, res) => { 
  run('UPDATE users SET money=money+? WHERE id=?', [req.body.amount, req.uid]); 
  res.json({ code: 200, msg: '充值成功' }); 
});

app.post('/api/pay/withdraw', auth, (req, res) => { 
  const { amount } = req.body; 
  const users = query('SELECT money FROM users WHERE id=?', [req.uid]); 
  if (users[0].money < amount) return res.json({ code: 400, msg: '余额不足' }); 
  run('UPDATE users SET money=money-? WHERE id=?', [amount, req.uid]); 
...
app.get('/api/admin/stats', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  res.json({ code: 200, data: { 
    userCount: query('SELECT COUNT(*) as cnt FROM users')[0]?.cnt || 0, 
    orderCount: query('SELECT COUNT(*) as cnt FROM run_orders')[0]?.cnt || 0, 
    totalMoney: query('SELECT SUM(fee) as total FROM run_orders WHERE status=2')[0]?.total || 0, 
    onlineCount: query('SELECT COUNT(*) as cnt FROM users WHERE online=1')[0]?.cnt || 0 
  } }); 
});

// Tenant management endpoints (CRUD)
app.get('/api/admin/tenants', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => {
  try {
...
app.get('/api/admin/migrations/current', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => {
  try {
...
app.post('/api/admin/tenants', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => {
  ...
  const id = uuidv4();
  try {
    run('INSERT INTO admin_tenants (id, name, status, description) VALUES (?,?,?,?)', [id, name, status, description]);
    res.json({ code: 200, msg: '创建成功', data: { id, name, status, description } });
  } catch (e) {
...
app.put('/api/admin/tenants/:id', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => {
  ...
  try {
    run('UPDATE admin_tenants SET name=?, status=?, description=? WHERE id=?', [name, status, description, id]);
    res.json({ code: 200, msg: '更新成功', data: { id, name, status, description } });
  } catch (e) {
...
app.get('/api/admin/users', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  const { page = 1, limit = 20 } = req.query; 
  res.json({ code: 200, data: { 
    list: query('SELECT * FROM users LIMIT ? OFFSET ?', [parseInt(limit), (parseInt(page) - 1) * parseInt(limit)]), 
    total: query('SELECT COUNT(*) as cnt FROM users')[0]?.cnt || 0 
  } }); 
});

app.post('/api/admin/user/freeze', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  const { userId, status } = req.body; 
  run('UPDATE users SET status=? WHERE id=?', [status, userId]); 
  res.json({ code: 200, msg: '操作成功' }); 
});

app.get('/api/admin/orders', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  res.json({ code: 200, data: query('SELECT r.*, u.nickname as publisher_name FROM run_orders r LEFT JOIN users u ON r.uid=u.id ORDER BY r.create_time DESC LIMIT 50') }); 
});

app.get('/api/admin/rider_auths', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  res.json({ code: 200, data: query('SELECT ra.*, u.nickname, u.avatar FROM rider_auth ra LEFT JOIN users u ON ra.uid=u.id WHERE ra.status=0') }); 
});

app.post('/api/admin/rider_auth/audit', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  run('UPDATE rider_auth SET status=? WHERE id=?', [req.body.status, req.body.authId]); 
  res.json({ code: 200, msg: '审核完成' }); 
});

app.get('/api/admin/groups', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  res.json({ code: 200, data: query('SELECT g.*, u.nickname as owner_name, (SELECT COUNT(*) FROM group_members WHERE group_id=g.id) as member_count FROM groups g LEFT JOIN users u ON g.owner_uid=u.id') }); 
});

app.post('/api/admin/group/dissolve', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  run('UPDATE groups SET is_dissolved=1 WHERE id=?', [req.body.groupId]); 
  run('DELETE FROM group_members WHERE group_id=?', [req.body.groupId]); 
  res.json({ code: 200, msg: '已解散' }); 
});

app.get('/api/admin/notices', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  res.json({ code: 200, data: query('SELECT * FROM notices ORDER BY create_time DESC') }); 
});

app.post('/api/admin/notices/add', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  run('INSERT INTO notices (title, content) VALUES (?,?)', [req.body.title, req.body.content]); 
  res.json({ code: 200, msg: '发布成功' }); 
});

app.get('/api/admin/banners', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  res.json({ code: 200, data: query('SELECT * FROM banners ORDER BY sort DESC') }); 
});

app.post('/api/admin/banners/add', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  const { title, image, url, sort = 0 } = req.body; 
  run('INSERT INTO banners (title, image, url, sort) VALUES (?,?,?,?)', [title, image, url, sort]); 
  res.json({ code: 200, msg: '添加成功' }); 
});

app.get('/api/admin/logs', adminAuth, requireAdminRole(['super_admin','tenant_admin']), (req, res) => { 
  res.json({ code: 200, data: query('SELECT l.*, a.username FROM logs l LEFT JOIN admins a ON l.admin_id=a.id ORDER BY l.create_time DESC LIMIT 50') }); 
});

app.get('/api/notices', auth, (req, res) => { 
  res.json({ code: 200, data: query('SELECT * FROM notices WHERE status=1 ORDER BY create_time DESC LIMIT 10') }); 
});

// ====================== 启动 ======================
initDatabase().then(() => {
  // 注释掉不存在的server_chat_phase1引入（避免启动报错）
